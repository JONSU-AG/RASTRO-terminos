...
// Límite simple anti-abuso por IP (protege la cuota de Gemini del autor sin dependencias extra)
const RATE_WINDOW_MS = 60 * 1000;
const RATE_MAX_REQUESTS = 30;
const rateBuckets = new Map<string, { count: number; resetAt: number }>();
function orsttyRateLimit(req: any, res: any, next: any) {
  const now = Date.now();
...
async function startServer() {
  ...
  app.use("/api/orstty", orsttyRateLimit);
  ...
  app.post("/api/orstty/chat", async (req, res) => {
    try {
      ...
      // Tope anti-abuso: mensajes gigantes disparan el costo de la IA
  ...
  app.post("/api/orstty/quick-quiz", async (req, res) => {
    try {
      ...
      const cleanSubject = String(subject).slice(0, 80);
      const ai = getGenAI();
      if (!ai) {
        return res.json({
      ...
      const response = await ai.models.generateContent({
        model: "gemini-3.6-flash",
        contents: `Genera 1 pregunta de opción múltiple tipo examen de admisión UNSA/CEPREUNSA para la materia: ${cleanSubject}.`,
        config: {
