...
export const AliadosCarousel = ({ onOpenAliadoForm }) => {
  ...
  useEffect(() => {
    ...
    const loadAllies = () => {
      ...
      // NOTA: el carrusel público SOLO muestra la tarjeta del Creador + usuarios con
      // más de 10 aportes. Las solicitudes de aliados se gestionan en el panel Admin
      // y ya no alimentan la vista pública.
  ...
  // NOTA: los autores destacados aportan sus materiales bajo las tarjetas existentes,
  // pero ya NO generan tarjetas nuevas de personas en la vista pública
  // (solo Creador + usuarios con más de 10 aportes).
  ...
  // Tope anti-saturación: se muestran 10 tarjetas con opción "Ver todos"
  const CAROUSEL_VISIBLE_LIMIT = 10;
  const [showAllCards, setShowAllCards] = useState(false);
  const displayCards = showAllCards ? carouselCards : carouselCards.slice(0, CAROUSEL_VISIBLE_LIMIT);
  ...
  return (
    <section style={{
      ...
      {carouselCards.length > CAROUSEL_VISIBLE_LIMIT && (
        <div style={{ display: 'flex', justifyContent: 'center', margin: '6px 0 2px' }}>
          <button
            onClick={() => setShowAllCards(v => !v)}
            style={{ padding: '10px 22px', borderRadius: '14px', border: '1.5px solid var(--card-border)', background: 'var(--card-bg)', color: 'var(--text-main)', fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer' }}
          >
            {showAllCards ? 'Ver menos' : `Ver todos (${carouselCards.length})`}
          </button>
        </div>
      )}
      {carouselLightbox && (
