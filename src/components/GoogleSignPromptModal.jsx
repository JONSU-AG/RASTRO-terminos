  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code !== 'auth/popup-closed-by-user') {
        // TEMP-DIAG: muestra el error tecnico real para diagnosticar el login nativo. Quitar antes de publicar.
        const tech = [err?.code, err?.message].filter(Boolean).join(' | ');
        setError(`Error al conectar con Google. Por favor, intenta de nuevo.${tech ? ` [Diag: ${tech}]` : ''}`);
      }
    } finally {
      setLoading(false);
    }
